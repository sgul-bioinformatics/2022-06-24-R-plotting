...
* We can change other stylistic parts of the chart with themes, we can set the general theme with some prebuilt styles e.g. `theme_bw`, `theme_minimal`, `theme_classic`, `theme_dark`; more details and examples can be found at https://ggplot2.tidyverse.org/reference/ggtheme.html
...
* the x-axis labels are not quite aligned to the tick marks, we can adjust them with `vjust` and `hjust`
...
### Plotting a scattor plot

* Lets plot a differnt type of graph, this time plotting `reads` against `coverage`
...
* we can easily add a smoothed regression line
...
* Finally if we wish we can make interactve versions of our plots using the `plotly` package
* try selection regions of the plot and zooimng,using the menu ribbon on the top right of the plot
